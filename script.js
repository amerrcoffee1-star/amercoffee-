// script.js - النسخة الشاملة مع إجبار الحد الأدنى (200 ج) وتوزيع الفروع وإصلاح البحث اللحظي
...
// =======================================================
// 7. خوارزمية البحث اللحظي (مثل جوجل)
// =======================================================
...
function filterStoreProducts(query) {
    ...
    // الانتقال لصفحة المتجر بمجرد كتابة أول حرف
    if (nQ.length > 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        // البحث اللحظي مع كل حرف (مثل جوجل)
        searchInput.addEventListener('input', (e) => {
            filterStoreProducts(e.target.value);
        });

        // دعم زر Enter لإخفاء الكيبورد أو تأكيد البحث
        searchInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                filterStoreProducts(e.target.value);
                searchInput.blur(); // إخفاء الكيبورد في الموبايل بعد الضغط على Enter
            }
        });
    }
