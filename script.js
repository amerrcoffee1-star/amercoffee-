// script.js - النسخة الشاملة مع إجبار الحد الأدنى (200 ج) وتوزيع الفروع
...
function navigate(pageId) {
    ...
    if (pageId === 'checkout') {
        if (!currentUser) { 
            ...
            pageId = 'store'; // إجبار العميل على البقاء في المتجر
        } else {
...
function renderCart() {
    ...
    // تعديل زر الدفع في السلة الجانبية (Sidebar) لمنع التحايل
...
function normalizeArabic(text) { 
    if (!text) return ''; let n = text.toLowerCase().trim(); 
    n = n.replace(/[أإآ]/g, 'ا').replace(/ة/g, 'ه').replace(/ى/g, 'ي').replace(/[\u064B-\u0652]/g, ''); return n; 
}

function filterStoreProducts(query) {
    const nQ = query ? normalizeArabic(query) : null;
    if (nQ) navigate('store');
    document.querySelectorAll('.store-product').forEach(card => {
        const nN = normalizeArabic(card.dataset.productName);
        card.style.display = (!nQ || nN.includes(nQ)) ? 'flex' : 'none';
    });
    document.querySelectorAll('.category-title').forEach(title => {
        const grid = document.querySelector(`[data-category-grid="${title.dataset.categoryName}"]`);
        title.style.display = (grid && Array.from(grid.querySelectorAll('.store-product')).some(i => i.style.display === 'flex')) ? 'block' : 'none';
    });
}

document.addEventListener('DOMContentLoaded', () => {
    updateUIForUser(); navigate('home'); 
    document.getElementById('search-input')?.addEventListener('input', (e) => filterStoreProducts(e.target.value));
