// script.js - النسخة الكاملة والمعدلة (مع تحديث السيرفر والبحث الاحترافي)

// 🚨🚨🚨 [ تحديث الرابط العالمي الجديد ] 🚨🚨🚨
const REMOTE_URL = 'https://4c8dd81faedba1ac8794f8f6e054c772.serveousercontent.com'; 
...
// =======================================================
// 1. البيانات الأساسية (Data Model) - كاملة بدون أي اختصار
// =======================================================
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            localStorage.setItem('amerrUser', JSON.stringify(currentUser));
            updateUIForUser();
            showToast(`مرحباً بك مجدداً يا ${currentUser.name}`, 'success');
            navigate('home');
...
async function confirmDeleteAccount() {
    ...
    try {
        ...
        if (res.ok && result.success) { 
            closeDynamicModal(); 
            showToast('تم حذف حسابك بنجاح. نأسف لرحيلك!', 'success'); 
...
function navigate(pageId) {
    ...
    if (pageId === 'checkout') {
        ...
        if (!currentUser) { showToast('يجب تسجيل الدخول لإتمام الطلب.', 'error'); pageId = 'login-page'; }
        else if (total < MINIMUM_ORDER_AMOUNT) { showToast(`الحد الأدنى للطلب ${MINIMUM_ORDER_AMOUNT} جنيهاً.`, 'error'); pageId = 'store'; }
    }
    setTimeout(() => {
...
function renderCart() {
    ...
    let total = 0;
    if (cart.length === 0) {
        const message = '<p style="text-align: center; margin-top: 20px; color: #777;">سلة التسوق فارغة حالياً.</p>';
...
async function submitOrderToServer(e) {
    e.preventDefault();
    if (!currentUser) return showToast('سجل دخولك أولاً لإتمام الطلب', 'error');
    ...
    try {
        ...
        else showToast('حدث خطأ أثناء إرسال الطلب، تأكد من اتصالك بالسيرفر', 'error');
    } catch (err) { showToast('خطأ: السيرفر لا يستجيب حالياً', 'error'); }
}
...
function showOrderSuccessModal(id) {
    const modalHtml = `
        <div id="dynamic-modal" class="modal-overlay" style="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); z-index:20000; align-items:center; justify-content:center; direction:rtl; text-align:center;">
            <div class="modal-card animate-in" style="background:white; padding:30px; border-radius:25px; width:90%; max-width:400px; box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
                ...
                <p style="color:#666; margin-bottom:20px;">تم تسجيل الطلب بنجاح. رقم تتبعك هو:</p>
                ...
                <button onclick="closeDynamicModal(); navigate('home');" style="width:100%; padding:14px; background:#5A3F33; color:white; border:none; border-radius:12px; font-weight:bold; cursor:pointer; transition: 0.3s;">العودة للرئيسية</button>
            </div>
        </div>`;
    document.body.insertAdjacentHTML('beforeend', modalHtml);
}
...
// =======================================================
// 7. وظيفة البحث والتهيئة (البحث الاحترافي)
// =======================================================
...
function filterStoreProducts(query) {
    ...
    // إذا بدأ المستخدم في الكتابة وهو ليس في صفحة المتجر، يتم تحويله فوراً
    const storeSection = document.getElementById('store');
    if (nQ && storeSection && storeSection.style.display !== 'block') {
        navigate('store');
    }
