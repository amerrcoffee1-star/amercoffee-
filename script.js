// script.js - النسخة الشاملة (دعم الطلب كضيف + إغلاق النوافذ باللمس الخارجي)
...
// =======================================================
// 2. الدوال العامة وإغلاق النوافذ باللمس الخارجي
// =======================================================
...
// إضافة ميزة الإغلاق عند الضغط في مكان فاضي
window.onclick = function(event) {
    const sideDrawer = document.getElementById('side-drawer');
    const cartSidebar = document.getElementById('cart-sidebar');
    const dynamicModal = document.getElementById('dynamic-modal');

    // إغلاق المنيو الجانبي
    if (event.target === sideDrawer) {
        closeSidebar();
    }
    // إغلاق سلة التسوق
    ...
    // إغلاق المودال (النافذة المنبثقة)
...
// =======================================================
// 4. إدارة التنقل - تعديل للسماح بالضيف (Guest)
// =======================================================
...
function navigate(pageId) {
    ...
    if (pageId === 'checkout') {
        // تم إلغاء شرط إجبار تسجيل الدخول هنا للسماح للضيف بالطلب
        if (total < MINIMUM_ORDER_AMOUNT) { 
            ...
            // تعبئة البيانات تلقائياً فقط إذا كان هناك مستخدم مسجل
...
async function submitOrderToServer(e) {
    ...
    // تم حذف شرط تسجيل الدخول للسماح للجميع بالطلب
