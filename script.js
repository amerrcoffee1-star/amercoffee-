// script.js - النسخة الكاملة والمعدلة (إصلاح الحجم والبحث وبيانات السيرفر)

// 🚨🚨🚨 [ بيانات الإعدادات والربط بالسيرفر ] 🚨🚨🚨
...
// =======================================================
// 1. البيانات الأساسية (Data Model) - [لم يتم تغيير سطر واحد هنا]
// =======================================================
        ...
        <div id="dynamic-modal" class="modal-overlay" style="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:10000; align-items:center; justify-content:center; direction:rtl; backdrop-filter:blur(4px);">
            <div class="modal-card animate-in" style="background:white; padding:25px; border-radius:20px; width:90%; max-width:400px; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
                ...
                <div style="line-height:1.8;">
                    <div style="display:flex; justify-content:space-between;"><span>الاسم:</span> <b>${currentUser.name}</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>الهاتف:</span> <b>${currentUser.phone}</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>الإيميل:</span> <b>${currentUser.email}</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>الجنسية:</span> <b>${currentUser.nationality}</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>العمر:</span> <b>${currentUser.age} عاماً</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>كلمة المرور:</span> <b><span id="pwd-field">********</span></b></div>
                    <div style="text-align:left;"><button onclick="document.getElementById('pwd-field').textContent = '${currentUser.password}'" style="background:none; border:none; color:#C78C4E; cursor:pointer; font-size:12px;">إظهار كلمة المرور</button></div>
                </div>
...
// ⚠️ تم إصلاح هذه الدالة لتعمل بشكل حقيقي مع السيرفر
async function confirmDeleteAccount() {
    try {
        // المسار الصحيح للسيرفر هو /api/users/ والباراميتر هو الإيميل
        ...
        if (res.ok && result.success) { 
            closeDynamicModal(); 
            showToast('تم حذف الحساب نهائياً من قاعدة البيانات', 'success'); 
            logout(); 
        } else {
            showToast(result.message || 'فشل الحذف من السيرفر', 'error');
        }
    } catch (e) { 
        showToast('تعذر الاتصال بالسيرفر لحذف الحساب', 'error'); 
    }
}
...
// =======================================================
// 6. إدارة السلة والأوردر - [تم تعديلها لإرسال الحجم والبيانات كاملة]
// =======================================================
...
async function submitOrderToServer(e) {
    ...
    try {
        ...
        } else { showToast('فشل في إرسال الطلب للسيرفر', 'error'); }
    ...
    const modalHtml = `
        <div id="dynamic-modal" class="modal-overlay" style="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); z-index:20000; align-items:center; justify-content:center; direction:rtl; text-align:center;">
            <div class="modal-card animate-in" style="background:white; padding:30px; border-radius:25px; width:90%; max-width:400px;">
                ...
                <p style="color:#666; margin-bottom:20px;">شكراً لثقتك في بُن عامر. رقم طلبك هو:</p>
                <div style="background:#f9f9f9; border:2px dashed #C78C4E; border-radius:15px; padding:15px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;">
                    <span id="order-id-txt" style="font-size:20px; font-weight:bold; color:#5A3F33;">${id}</span>
                    <button onclick="copyToClipboard('${id}')" style="background:#C78C4E; color:white; border:none; padding:8px 12px; border-radius:8px; cursor:pointer; font-size:12px; display:flex; align-items:center; gap:5px;"><i class="material-icons" style="font-size:16px;">content_copy</i> نسخ</button>
                </div>
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(() => showToast('تم نسخ رقم الطلب بنجاح!', 'success'));
}

// =======================================================
// 7. وظيفة البحث والفلترة - [نسخة محسنة]
// =======================================================
...
function filterStoreProducts(query) {
    ...
    document.querySelectorAll('.category-title').forEach(title => {
        ...
        if (grid) {
            const hasVisibleItems = Array.from(grid.querySelectorAll('.store-product')).some(p => p.style.display !== 'none');
...
function handleGlobalSearch() {
    ...
    if (q.length < 2) { showToast('يرجى إدخال حرفين على الأقل للبحث', 'info'); return; }

    navigate('store');
    setTimeout(() => {
        const count = filterStoreProducts(q);
        if (count === 0) showToast('عذراً، لا توجد نتائج مطابقة', 'info');
        else showToast(`تم العثور على ${count} منتج`, 'success');
        searchInput.value = '';
    }, 400); 
}
